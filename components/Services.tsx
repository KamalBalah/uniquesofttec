import { IServicesList } from "@/interfaces";
import { useTranslations } from "next-intl";
import Image from "next/image";

interface IProps {
  services: IServicesList[];
}

const Services = ({ services }: IProps) => {
  const t = useTranslations("services");
  return (
    <div id="services" className="text-main pt-10">
      <div className="h-full overflow-y-auto scrollbar-hide pb-22">
        <h2 className="text-3xl text-green text-center font-bold">
          {t("sectionTitle")}
        </h2>
        <p className="text-base text-center my-2">{t("description")}</p>

        <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto">
          {services.map((service, index) => (
            <div
              key={index}
              className="bg-background/70 text-center transition-all duration-300 rounded-md p-6 border border-border/50  hover:border-border shadow-sm hover:shadow-lg"
            >
              <div className="text-5xl mb-4 flex justify-center p-3 bg-green/20 w-14 aspect-square m-auto rounded-md">
                <Image
                  src={`/${service.icon}`}
                  width={32}
                  height={32}
                  alt={service.title}
                  color="red"
                />
              </div>
              <h3 className="text-lg font-semibold text-main">
                {t(service.title)}
              </h3>
              <p className="text-sm text-gray-400 mt-2">{t(service.desc)}</p>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default Services;
